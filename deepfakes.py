...
from voice_conversion.audio_seg import audio_slicer
...
def tts(
    ...
    input_path = "./data/1_input/text"
    output_dir = "./data/2_text_to_speech_audio"
...
@click.option(
    "--input-dir",
    type=click.Path(exists=True),
    default="./data/5_voice_conversion_train/1_input_audio",
    help="path to dataset",
)
@click.option(
    "--filelist-path",
    type=click.Path(),
    default="./data/5_voice_conversion_train/filelists",
    help="path to file lists",
)
@click.option(
    "--config_path",
    type=click.Path(),
    default="./data/5_voice_conversion_train/configs/config.json",
    help="path to config.json",
)
@click.option(
    "--cache_dir",
    type=click.Path(),
    default="./data/5_voice_conversion_train/cache",
    help="path to temporary folder",
)
def vc_pre_processing(input_dir, filelist_path, config_path, cache_dir):
    dataset_folder = os.path.abspath("./data/5_voice_conversion_train/dataset_raw")
    # # clear dataset folder
    os.system(f"rm -rf {dataset_folder}/*")
    
    output_slicer = os.path.join(dataset_folder, "slicer")
    Path(output_slicer).mkdir(parents=True, exist_ok=True)
    audio_slicer(input_dir=input_dir, output_dir=output_slicer)
...
@click.option(
    "--model-path",
    type=click.Path(),
    default="./data/5_voice_conversion_train/model",
    help="path to model",
)
@click.option(
    "--max-epoch",
...
@click.option(
    "--input-path",
    type=click.Path(exists=True),
    default="./data/2_text_to_speech_audio",
    help="path to input dir or file",
)
@click.option(
    "--output-dir",
    type=click.Path(),
    default="./data/3_voice_conversion_audio",
    help="path to output dir",
)
@click.option(
    "--model-path",
...
@click.option(
    "--config-path",
    type=click.Path(),
    default="./data/5_voice_conversion_train/model/config.json",
    help="path to config.json",
)
def vc_infer(
    input_path: Path,
...
@click.option(
    "--video-path",
    type=click.Path(),
    default="./data/1_input/video",
    help="path to input video dir or video file",
)
@click.option(
    "--output-dir",
    type=click.Path(exists=True),
    default="./data/4_output_lip_sync_video",
    help="path to output dir",
)
