...
def inference(face_path, audio_path, outfile, cache_dir, start_frame = None):
    ...
    os.makedirs(os.path.join(cache_dir, args.tmp_dir), exist_ok=True)

    enhancer = FaceEnhancement(base_dir=os.path.join(base_dir,'checkpoints'), size=512, model='GPEN-BFR-512', use_sr=False, \
                               sr_model='rrdb_realesrnet_psnr', channel_multiplier=2, narrow=1, device=device)
    restorer = GFPGANer(model_path=os.path.join(base_dir,'checkpoints/GFPGANv1.3.pth'), upscale=1, arch='clean', \
    ...
    croper = Croper(os.path.join(base_dir,'checkpoints/shape_predictor_68_face_landmarks.dat'))
    ...
    if not os.path.isfile(cache_dir+'/'+base_name+'_landmarks.txt') or args.re_preprocess:
        print('[Step 1] Landmarks Extraction in Video.')
        kp_extractor = KeypointExtractor()
        lm = kp_extractor.extract_keypoint(frames_pil, cache_dir+'/'+base_name+'_landmarks.txt')
    else:
        print('[Step 1] Using saved landmarks.')
        lm = np.loadtxt(cache_dir+'/'+base_name+'_landmarks.txt').astype(np.float32)
        lm = lm.reshape([len(full_frames), -1, 2])
   
    if not os.path.isfile(cache_dir+'/'+base_name+'_coeffs.npy') or args.re_preprocess:
        net_recon = load_face3d_net(args.face3d_net_path, device)
        lm3d_std = load_lm3d(base_dir+'/checkpoints/BFM')
        ...
        np.save(cache_dir+'/'+base_name+'_coeffs.npy', semantic_npy)
    else:
        print('[Step 2] Using saved coeffs.')
        semantic_npy = np.load(cache_dir+'/'+base_name+'_coeffs.npy').astype(np.float32)
    ...
    if args.exp_img is not None and ('.png' in args.exp_img or '.jpg' in args.exp_img):
        ...
        W, H = exp_pil.size
        kp_extractor = KeypointExtractor()
        lm_exp = kp_extractor.extract_keypoint([exp_pil], cache_dir+'/'+base_name+'_temp.txt')[0]
        if np.mean(lm_exp) == -1:
        ...
        expression = torch.tensor(loadmat(base_dir+'/checkpoints/expression.mat')['expression_mouth'])[0]
    else:
        print('using expression center')
        expression = torch.tensor(loadmat(base_dir+'/checkpoints/expression.mat')['expression_center'])[0]

    # load DNet, model(LNet and ENet)
    D_Net, model = load_model(args, device)

    if not os.path.isfile(cache_dir+'/'+base_name+'_stablized.npy') or args.re_preprocess:
        imgs = []
        ...
        np.save(cache_dir+'/'+base_name+'_stablized.npy',imgs)
        del D_Net
    else:
        print('[Step 3] Using saved stabilized video.')
        imgs = np.load(cache_dir+'/'+base_name+'_stablized.npy')
    torch.cuda.empty_cache()

    if not args.audio.endswith('.wav'):
        command = 'ffmpeg -loglevel error -y -i {} -strict -2 {}'.format(args.audio, cache_dir+'/{}/temp.wav'.format(args.tmp_dir))
        subprocess.call(command, shell=True)
        args.audio = cache_dir+'/{}/temp.wav'.format(args.tmp_dir)
    ...
    lm = lm[start:start+len(mel_chunks)]
    ...
    gen = datagen(imgs_enhanced.copy(), mel_chunks, full_frames, None, (oy1,oy2,ox1,ox2), cache_dir)

    frame_h, frame_w = full_frames[0].shape[:-1]
    out = cv2.VideoWriter(cache_dir+'/{}/result.mp4'.format(args.tmp_dir), cv2.VideoWriter_fourcc(*'mp4v'), fps, (frame_w, frame_h))
    ...
    command = 'ffmpeg -loglevel error -y -i {} -i {} -strict -2 -q:v 1 {}'.format(args.audio, cache_dir+'/{}/result.mp4'.format(args.tmp_dir), args.outfile)
    subprocess.call(command, shell=platform.system() != 'Windows')
    print('outfile:', args.outfile)


# frames:256x256, full_frames: original size
def datagen(frames, mels, full_frames, frames_pil, cox, cache_dir):
    ...
    lms = kp_extractor.extract_keypoint(fr_pil, cache_dir+'/'+base_name+'x12_landmarks.txt')
...
if __name__ == '__main__':
    inference(face_path = args.face, audio_path = args.audio, outfile = args.outfile)
